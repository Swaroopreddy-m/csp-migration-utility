...
public class ResourceResolver {
    ...
    public static Path resolveResource(String srcPath, Path currentPageFile, Path htmlRoot, String contextPath) {
        // Default: do not prompt if missing (used during verification)
        return resolveResource(srcPath, currentPageFile, htmlRoot, contextPath, true, false);
    }

    public static Path resolveResource(String srcPath, Path currentPageFile, Path htmlRoot, String contextPath, boolean isScript, boolean promptIfMissing) {
        ...
        int choice;
        if (isScript) {
            choice = InputManager.promptMissingScript(src);
        } else {
            choice = InputManager.promptMissingStylesheet(src);
        }
